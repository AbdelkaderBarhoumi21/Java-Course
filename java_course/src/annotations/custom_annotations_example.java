 ...
 *
 * .class in Java — quick overview:
 *
 * Car.class is a Class Literal. It gives you the Class object that represents
 * the Car class itself — not an instance, but the blueprint/metadata of Car.
 *
 *   Car myCar = new Car();   // an INSTANCE of Car  → the actual object
 *   Car.class                // the CLASS itself     → metadata about Car
 *
 ...
 *   info.getName();                   → "Car"
 *   info.getDeclaredFields();         → [model, color]
 *   info.getDeclaredMethods();        → [drive, stop]
 *   info.getAnnotation(Author.class); → reads @Author
 *   info.getSuperclass();             → Object
 *
 * This is called Reflection — inspecting a class while the program is running.
 *
